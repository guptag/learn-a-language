...
export class Heap<T> implements IHeap<T> {
  ...
  private bubbleUp(index: number): void {
    ...
    if (parent === undefined || parentIndex === undefined) {
      return;
    }

    if (
      (this.heapType === HeapType.Min && this.compareFn(parent, current) > 0) ||
  ...
  private bubbleDown(index: number): void {
    ...
    if (firstChild === undefined || firstChildIndex === undefined) {
      return;
    }

    if (
      (this.heapType === HeapType.Min &&
        this.compareFn(current, firstChild) > 0) ||
  ...
  private getFirstChild(index: number): T | undefined {
    ...
    return childIndex < this.queue.length
      ? this.queue[2 * index + 1]
      : undefined;
  }
  ...
  private getParent(index: number): T | undefined {
    ...
    return parentIndex < this.queue.length
      ? this.queue[Math.floor(index / 2)]
      : undefined;
  }
  ...
  private swap(index1: number, index2: number) {
    if (index1 >= this.queue.length || index2 >= this.queue.length) {
      return;
    }
