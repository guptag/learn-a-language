...
function sort<T>(list: T[], begin: number, end: number, compareFn?: ICompare<T>) {
  if (begin <= end) { return; }
...
function merge<T>(list: T[], begin: number, mid: number, end: number, compareFn?: ICompare<T>): void {
  const list1: T[] = [];
  const list2: T[] = [];
  ...
  for (let i = begin; i <= end; ++i) {
    if (list1Index >= list1.length) {
      ...
      list[i] = (compareFn(list1[list1Index], list2[list2Index]) < 0) ? list1[list1Index++] : list2[list2Index++];
    }
  }
}
