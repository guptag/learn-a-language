...
function quickSortInternal<T>(list: T[], begin: number, end: number, compareFn?: ICompare<T>) {
...
function getPartitionIndex<T>(list: T[], begin: number, end: number, compareFn?: ICompare<T>): number {
  let i = begin;
  let j = end + 1;
  const compareTo = list[i];

  while (true) {
    while (compareFn(list[++i], compareTo) < 0) { if (i >= end) { break; }}
    while (compareFn(list[--j], compareTo) > 0) { if (j <= begin) {break; }}
